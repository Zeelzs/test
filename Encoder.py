...
class CompilerHelper:
    # Updated symbols to include delimiters and avoid using ^, * for multiplication
    symbols = [
        '☺', '♔', '♕', '♖', '♗', '♘', '♙', '♚', '♛', '♜', '♝', '♞', '♟', '♠', '♡', '♢', '♣', '♤', '♥', '♦', '♧',
        '♨', '♩', '♪', '♫', '♬', '♭', '♮', '♯', '✁', '✂', '✃', '✄', '✆', '✇', '✈', '✉', '✊', '✋', '✌', '✍', '✎',
        '✏', '✐', '✑', '✒', '✓', '✔', '✕', '✖', '✗', '✘', '✙', '✚', '✛', '✜', '✝', '✞', '✟', '✠', '✡', '✢', '✣',
    ...
    # Delimiters for R, G, B components and count
    R_DELIM = '^'
    G_DELIM = '&'
    B_DELIM = '*'
    COUNT_DELIM = ']'
        ...
        for x in range(width):
            ...
            if color_triplet == current_color:
                ...
                if current_color is not None:
                    # Multiply run_length by 3 as per user instruction
...
def decode_and_save_image(file_path, width, height, output_image):
    ...
    for line in compressed_lines:
        ...
        # Split row number and encoded data
        ...
        while i < len(data) and x < width:
            # Extract color triplet (3 symbols)
            if i + 3 > len(data):
                break
            ...
            # Extract count (digits until COUNT_DELIM)
            count_str = ''
